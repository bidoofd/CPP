...
void Tree::insertNodeAuto(string value, Node* currentNode)
{
    //create new node
    Node* newNode = new Node(value);
    //if the root is null set the root to new node value
    ...
    else
    {
        if(currentNode->left != NULL)
        {
            insertNodeAuto(value, currentNode->left);
        }
        currentNode->data = value;
        if(currentNode->right != NULL)
        {
            insertNodeAuto(value, currentNode->right);
        }
...
void Tree::expressOptionTree(string word, Node* root)
{
    for(int a = 0; a < word.length(); a++)
    {
        string temp(1, word[a]);
        insertNodeAuto(temp, this->root);
    }
}
